...
public final class ScreenLock {
  ...
  public void release(Object currentOwner) {
    lock.lock();
    try {
      if (!acquired) throw noLockToRelease();
      if (owner != currentOwner) throw wrongOwner(currentOwner);
      acquired = false;
      owner = null;
      released.signal();
    } finally {
  ...
  private static ScreenLockException noLockToRelease() {
    return new ScreenLockException("No lock to release");
  }

  private static ScreenLockException wrongOwner(Object currentOwner) {
    String message = String.format("<%s> is not the lock owner", currentOwner);
    return new ScreenLockException(message);
  }
   ...
   * @return {@code true} if the given object is owning the lock; <code>false</code> otherwise.
   */
   ...
   * @return {@code true} if the lock is already acquired; <code>false</code> otherwise.
